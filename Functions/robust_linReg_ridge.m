function [theta_new, p_new, Theta, P] = robust_linReg_ridge(x_aug,y_aug,theta_old,epsilon, lambda)
%INPUT
%x_aug : n x d matrix
%y_aug : n x 1 vector
%epsilon : scalar
...
% optimize
while (cnt==1)
    alpha = (y_aug-x_aug*theta_old).^2 + lambda.*sum(theta_old.^2,1);
    ...
    theta_new  = (x_aug'*(p_new.*x_aug) + lambda)\(x_aug'*(p_new.*y_aug));
